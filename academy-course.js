(() => {
  ...
  document.querySelector('#course-page').innerHTML = `
    ...
    <section class="course-hero">
      <div class="course-hero__copy"><p class="course-kicker">Arden Academy · ${course.category}</p><h1>${course.name}</h1><p class="course-lead">${course.short}</p><div class="course-hero__meta"><span>Small-group learning</span><span>Guided practical work</span><span>Certificate pathway</span></div><dl><div><dt>Duration</dt><dd>${course.duration}</dd></div><div><dt>Location</dt><dd>Lalitpur</dd></div></dl><div class="course-actions"><a class="course-button course-button--primary" href="${whatsapp}" target="_blank" rel="noopener">Enrol now <span>↗</span></a><a class="course-button" href="contact.html">Contact Academy</a></div></div>
      <figure class="course-hero__visual"><div class="course-hero__frame"><img src="IMAGES/${course.image}" alt="${course.name} at Arden Academy Lalitpur"><span class="course-hero__seal"><b>Arden</b><small>Academy</small></span></div><figcaption><span>Professional training</span><span>Arden · Lalitpur</span></figcaption></figure>
    </section>
    <section class="course-overview"><div><p class="course-kicker">The course</p><h2>About This <em>Course</em></h2></div><div class="course-overview__copy"><p>${course.about[0]}</p><p>${course.about[1]}</p><div class="course-signature">Technique · Practice · Confidence</div></div></section>
    ...
    ${includes}
    <section class="course-experience"><p class="course-kicker">The Arden Academy experience</p><h2>Learn. Practice.<br><em>Perfect.</em></h2><p>Develop professional beauty skills through focused instruction, demonstrations and hands-on practice at Arden Academy.</p></section>
